...
В данной лабораторной работе исследуется и реализуется алгоритм параллельного вычисления префиксной суммы (scan) на графическом процессоре с использованием технологии CUDA. Префиксная сумма является одной из фундаментальных операций параллельных вычислений и широко применяется в задачах обработки данных, компьютерной графики, сортировки, анализа массивов и других областях.

Целью работы является разработка эффективной реализации префиксной суммы для одномерного массива с использованием параллелизма GPU. В рамках работы необходимо реализовать ядро CUDA, обеспечивающее корректное и производительное вычисление префиксной суммы для входных данных произвольного размера (в пределах ограничений задачи).

Особое внимание уделяется оптимизации доступа к памяти, в частности использованию разделяемой памяти, а также корректной обработке граничных условий. В ходе выполнения работы также анализируются вычислительная сложность алгоритма, количество операций и обращений к памяти, а также возможности дальнейшей оптимизации.

\section{Цель работы}

Освоение освоение методов выполнения инклюзивного сканирования на GPU. Получение навыков расчета префиксных сумм одномерного списка с помощью CUDA.

\section{Задание}

Реализовать программу, выполняющую расчет префиксных сумм одномерного списка на GPU с помощью CUDA.

\section{Ход работы}

В листинге ниже приведено содержание файла rvs\_laboratories\_13/\-rvs\_lab\-orat\-ories\_13.cu, который содержит код, выполняющую расчет префиксных сумм одномерного списка на GPU с помощью CUDA (лист. \ref{lst:main}).

\begin{code}    
	\caption{Содержание rvs\_laboratories\_13/rvs\_laboratories\_13.cu\label{lst:main}}
	\inputminted{c++}{../rvs_laboratories_13/rvs_laboratories_13.cu}
\end{code}

В листинге ниже приведено содержание файла dataset\_generator/data\-set\-\_gen\-er\-at\-or.cpp, который содержит код, генерирующий тестовые входные и контрольные выходные данные (лист. \ref{lst:dataset_generator}).

\begin{code}    
	\caption{Содержание dataset\_generator/dataset\_generator.cpp\label{lst:dataset_generator}}
	\inputminted{c++}{../dataset_generator/dataset_generator.cpp}
\end{code}
...
Как можно видеть, вычисление префиксных сумм было выполнено корректно и успешно.

\sectionc{Заключение}

Таким образом, в ходе выполнения данной лабораторной работы были освоены методы выполнения инклюзивного сканирования на GPU и получены навыки расчета префиксных сумм одномерного списка с использованием CUDA. На практике была реализована программа, выполняющая расчет префиксных сумм одномерного списка на GPU с помощью CUDA.
